		...
		function html()
		{
			...
			foreach($this->loop as $entry)
			{
				...
				if($this->atts['caption_elements'] != 'none' || !empty($text_add))
				{
                    ...
                    $items .=	$post_details->geodir_email;
					...
					if(strpos($this->atts['caption_elements'], 'excerpt') !== false && !empty($content)){
						...
						$items .=	"<div class='av-masonry-entry-content entry-content' {$markup}><b>{$content}</b></div>";
					}
					$items .=	$text_after."</div></figcaption><?php echo $post_details?>";
