			...
			foreach($this->loop as $entry)
			{
				...
				if($this->atts['caption_elements'] != 'none' || !empty($text_add))
				{
					...
					if(strpos($this->atts['caption_elements'], 'excerpt') !== false && !empty($content)){
                        ...
                        console.log($entry);
						$items .=	"<div class='av-masonry-entry-content entry-content' {$markup}><script>console.log(<? echo "script here"; ?>);</script><?php get_post_field( $fields_location, id); ?>{$content}</div>";
					}
